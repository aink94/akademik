...
class Auth extends CI_Controller {
	...
	function index(){
		...
		if($this->form_validation->run()){
			...
			$th_ajaran = $this->db->query("SELECT * FROM th_ajaran WHERE STATUS='aktif';")->row();
			$log = $this->M_auth->login_m($nupk, $pass);
			if($log->num_rows()==1){
				$log = $log->row_array();
				$login = array(
							'nupk'=>$log['nupk_guru'], 
							'username'=>$log['nama_guru'], 
							'akses'=>$log['jabatan_guru'],
							'pangkat'=>$log['pangkat_guru'],
							'foto'=>$log['foto'],
							'th_ajaran_aktif'=> $th_ajaran->id_th_ajaran,
							'login'=>TRUE
				);
